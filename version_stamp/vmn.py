...
class IVersionsStamper(object):
    ...
    def initialize_backend_attrs(self):
        ...
        seen = set()
        version_files_to_track_diff = [
            x for x in version_files_to_track_diff if not (x in seen or seen.add(x))
        ]
    ...
    def write_version_to_file(self, version_number: str) -> None:
        if self.dry_run:
            stamp_utils.VMN_LOGGER.info(
                f"Would have written to version file:\nversion: {version_number}\n"
            )
        else:
    ...
    def create_config_files(self):
        ...
        if not self.conf_file_exists:
            ...
            with open(self.app_conf_path, "w+") as f:
                msg = "# Autogenerated by vmn. You can edit this configuration file\n"
                f.write(msg)
...
class VersionControlStamper(IVersionsStamper):
    ...
    def update_stamping_info(
        ...
        self.current_version_info["stamping"]["app"]["stamped_on_remote_branch"] = (
...
def handle_stamp(vmn_ctx):
    ...
    if vmn_ctx.vcs.tracked and vmn_ctx.vcs.release_mode is None:
        vmn_ctx.vcs.current_version_info["stamping"]["app"]["release_mode"] = (
...
def handle_release(vmn_ctx):
    ...
    if vmn_ctx.args.stamp:
        ...
        if "detached" in status["state"]:
            stamp_utils.VMN_LOGGER.error("Cannot use --stamp in detached HEAD state")
            return 1

        # Deps must be clean — same requirement as regular stamp
        if status["dirty_deps"]:
            stamp_utils.VMN_LOGGER.error("Cannot use --stamp with dirty dependencies")
            return 1
    ...
    if ver:
        ...
        if props["buildmetadata"] is not None:
            stamp_utils.VMN_LOGGER.error(
                f"Failed to release {ver}. Releasing metadata versions is not supported"
            )
...
def _stamp_version(versions_be_ifc, pull, check_vmn_version, verstr):
    ...
    while retries:
        ...
        if err == 1:
            ...
            stamp_utils.VMN_LOGGER.warning(
                "Failed to publish. Will try to auto-increase from {0} to {1}".format(
                    current_version,
...
def vmn_run(command_line=None):
    ...
    try:
        ...
        stamp_utils.VMN_LOGGER.error(
            "Failed to init logger. Maybe you are running from a non-managed directory?"
        )
        stamp_utils.VMN_LOGGER.debug("Logged exception: ", exc_info=True)
...
def add_arg_release(subprasers):
    ...
    group.add_argument(
        ...
        default=None,
        required=False,
        help=f"The version to release in the format:  {stamp_utils.VMN_VERSION_FORMAT}",
    )
...
def add_arg_goto(subprasers):
    ...
    pgoto.add_argument(
        ...
        default=None,
        required=False,
        help=f"The version to go to in the format:  {stamp_utils.VMN_VERSION_FORMAT}",
    )
